...
<div id="subPageBox">
			...
			<tr>
				<td>
					...
					<table align="left">
						...
						<tr>
							<td><label>전송방법</label></td>
							<td>
								<select id="clMethod" name="clMethod">
									<option value="0">::::: 전체 :::::</option>
									<% for i = 0 to ubound(arrCallMethod) %>
										<option value="<%=i+1%>"><%=arrCallMethod(i)%></option>
									<% next %>
								</select>
							</td>
							<td>&nbsp;&nbsp;</td>
	...
	<%
	arrListHeader = array("구분","제목","발신자","대상","전송요청일시","상태","상세보기","삭제")
	arrListWidth = array("80px","*","100px","80px","160px","80px","100px","60px")
...
<script>
	...
	function fnLoadPage(p){
		page = p;
		var clMethod = $('#clMethod').val();
		var clStep = $('#clStep').val();
		var param = 'proc=CallResult&param=E]|[0]|['+clMethod+']|['+clStep+']|[0]|[0]|[0]|['+$('#schSDate').val()+']|['+$('#schEDate').val()+']|['+$('#schKey').val()+']|['+$('#schVal').val()+'&page='+page+'&pageSize='+pageSize;
		...
		if(rowCnt > 0){
			...
			for(i = 2; i < arrList.length; i++){
				...
				//	CL_IDX(2), AD_IDX(3), AD_ID(4), MSG_IDX(5), MSG_GB(6), CD_MSGTP(7), CDMSGTPNM(8), CL_METHOD(9), CL_TRY(10), CL_RSVDT(11), CL_SMSGB(12), CL_VMSGB(13), CL_FMSGB(14), CL_SNDNUM(15), CL_TIT(16), CL_STEP(17), TRGCNT(18), REGDT(19)
				...
				+'	<td class="aC"><img class="imgBtn" src="<%=pth_pubImg%>/btn/red_del2.png" title="삭제" onclick="fnCallDel('+arrVal[2]+')" /></td>'
				+'</tr>';
	...
	function fnDetailView(idx){
		layerW = 1000;
		layerH = 760;
		var url = '/pages/result/pop_resultView.asp?gb=E&clIdx='+idx;
		fnOpenLayer('전송결과 상세보기',url);
	}
	
	function fnCallDel(idx){
		if(confirm('삭제하시겠습니까?')){
			procFrame.location.href = 'callDel.asp?clIdx='+idx;
		}
	}
