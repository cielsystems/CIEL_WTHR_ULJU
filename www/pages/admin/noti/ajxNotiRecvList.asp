...
dim schSDate				: schSDate				= fnIsNull(nFnReq("schSDate", 10), fnDateToStr(dateserial(year(date),month(date),1), "yyyy-mm-dd"))
dim schEDate				: schEDate				= fnIsNull(nFnReq("schEDate", 10), fnDateToStr(dateadd("d",0,date), "yyyy-mm-dd"))
...
'response.write	" exec nusp_listNotiRecv '" & warnVarCode & "', " & warnStressCode & ", '" & schSDate & "', '" & schEDate & "', '" & schKey & "', '" & schVal & "', " & page & ", " & pageSize & ", " & ss_userIndx & ", '" & svr_remoteAddr & "' "

set cmd = server.createobject("adodb.command")
with cmd

	.activeconnection = strDBConn
	.commandtext = "nusp_listNotiRecv"
	...
	.parameters.append .createParameter("@schSDate",				adVarchar,	adParamInput,		10)
	.parameters.append .createParameter("@schEDate",				adVarchar,	adParamInput,		10)
	...
	.parameters("@schSDate")				= schSDate
	.parameters("@schEDate")				= schEDate
