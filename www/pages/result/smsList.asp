...
<div id="subPageBox">
	...
	<%
	arrListHeader = array("제목","구분","대상","전송요청일시","상태","상세보기")
	arrListWidth = array("*","80px","100px","160px","80px","100px")
...
<script>
	...
	function fnLoadPage(p){
		...
		var param = 'proc=CallResult&param=S]|[<%=ss_userIdx%>]|['+clMethod+']|['+clStep+']|[0]|[0]|[0]|['+$('#schSDate').val()+']|['+$('#schEDate').val()+']|['+$('#schKey').val()+']|['+$('#schVal').val()+'&page='+page+'&pageSize='+pageSize;
		...
		if(rowCnt > 0){
			var strRow, arrVal, clStep, smsGB = '-';
			for(i = 2; i < arrList.length; i++){
				arrVal = arrList[i].split(']|[');
				//	CL_IDX(2), AD_IDX(3), AD_ID(4), MSG_IDX(5), MSG_GB(6), CD_MSGTP(7), CDMSGTPNM(8), CL_METHOD(9), CL_TRY(10), CL_RSVDT(11), CL_SMSGB(12), CL_VMSGB(13), CL_FMSGB(14), CL_SNDNUM(15), CL_TIT(16), CL_STEP(17), TRGCNT(18), REGDT(19), SMSGB(20)
				...
				if(arrVal[20] == 'S'){smsGB = '<span class="colGreen">단문</span>';
				}else if(arrVal[20] == 'L'){smsGB = '<span class="colOrange">장문</span>';
				}else if(arrVal[20] == 'M'){smsGB = '<span class="colPurple">멀티</span>';
				}else if(arrVal[20] == 'B'){smsGB = '<span class="colBlue">매크로</span>';
				}
				strRow = '<tr>'
				+'	<td class="aL">'+arrVal[16]+'</td>'
				+'	<td class="aC">'+smsGB+'</td>'
	...
	function fnDetailView(idx){
		layerW = 1000;
		layerH = 700;
		var url = 'pop_resultView.asp?gb=S&clIdx='+idx;
