...
%>
<table border="0"><tr><td colspan="13"><h1>비상 발령 결과 보고서</h1></td></tr></table>
<br />
<table border="1">
	<tr>
		<th>발령코드</th>
		<td colspan="5"><%=clCode%></td>
		<th>발령자계정</th>
		<td colspan="3"><%=adNM%>(<%=adID%>)</td>
		<th>발령결과</th>
		<td colspan="2"><%=arrCallStep(clStep)%></td>
	</tr>
	<tr>
		<th>(음성)시작시간</th>
		<td colspan="5"><%=clVMSSDT%></td>
		<th>완료시간</th>
		<td colspan="3"><%=clVMSEDT%></td>
		<th>소요시간</th>
		<td colspan="2"><%=fnPeriodToStr(clVMSSDT, clVMSEDT)%></td>
	</tr>
	<tr>
		<th>(문자)시작시간</th>
		<td colspan="5"><%=clSMSSDT%></td>
		<th>완료시간</th>
		<td colspan="3"><%=clSMSEDT%></td>
		<th>소요시간</th>
		<td colspan="2"><%=fnPeriodToStr(clSMSSDT, clSMSEDT)%></td>
	</tr>
</table>
<br />
<table border="1">
	<tr>
		<th>음성내용</th>
		<td colspan="12"><%=clVMSMsg%></td>
	</tr>
	<tr>
		<th>문자내용</th>
		<td colspan="12"><%=clSMSMsg%></td>
	</tr>
</table>
<br />
<table border="1">
	<tr>
		<th rowspan="2">전체</th>
		<th colspan="7">음성발령결과(회신번호:<%=clSndNum1%>)</th>
		<th colspan="5">문자발령결과(회신번호:<%=clSndNum2%>)</th>
	</tr>
	<tr>
		<th>음성발령</th>
		<th colspan="2">응답</th>
	...
	<tr>
		...
		<td colspan="2"><%=formatNumber(cntVMSAnsw,0)%></td>
		<td colspan="2"><%=formatNumber(cntVMSCmp - cntVMSAnsw,0)%></td>
		...
		<td><%=formatNumber(cntSMSCmp - cntSMSAnsw,0)%></td>
		<td><%=formatNumber(cntSMSStay + cntSMSIng,0)%></td>
		<td><%=formatNumber(cntSMSCnl + cntSMSErr,0)%></td>
	</tr>
</table>
<br />
<table border="0"><tr><td colspan="13"><h2>비상 발령 대상자 목록</h2></td></tr></table>
<br />
<%
'#	상단 보고서 : 끝	========================================================
...
next
response.write	"		<th>문자시작일시</th>"
response.write	"		<th>문자완료일시</th>"
response.write	"		<th>음성시작일시</th>"
response.write	"		<th>음성완료일시</th>"
...
for i = 0 to arrRc2
	...
	response.write	"	<td class=""txt"">" & arrRs(26,i) & "</td>"
	response.write	"	<td class=""txt"">" & arrRs(27,i) & "</td>"
