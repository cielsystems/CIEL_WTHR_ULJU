...
for i = 0 to ubound(arrTrg)
	...
	if fnChkMobileNum(trim(arrVal(2))) = true or fnChkPhoneNum(trim(arrVal(2))) = true then
		
		sql = sql & " ( "
		sql = sql & " " & ss_userIdx & " "
		sql = sql & " , '" & svr_remoteAddr & "' "
		sql = sql & " , " & clng(maxNo) + i & " "
		sql = sql & " , " & clng(maxSort) + i & " "
		sql = sql & " , " & trim(arrVal(0)) & " "
		sql = sql & " , '" & trim(arrVal(1)) & "' "
...
if cntOK > 0 then
	...
	if cntErr > 0 then
		response.write	"alert('전화번호가 잘못된 " & cntErr & "명을 제외한 " & cntOK & "명의 대상자가 추가되었습니다.');"
	else
