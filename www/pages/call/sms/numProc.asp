...
if proc = "input" then
	...
	sql = sql & " values (" & ss_userIdx & ", '" & svr_remoteAddr & "', " & maxNo & ", " & maxSort & ", 0, '" & addNm & "' "
	'sql = sql & " , dbo.ecl_ENCRPART('" & addNum & "',4) "
	sql = sql & " , '" & addNum & "' "
	sql = sql & " ) "
	response.write	sql
	execSql(sql)
...
elseif proc = "delNum" then
	...
	sql = " delete from TMP_CALLTRG where TMP_NO = '" & no & "' and AD_IDX = " & ss_userIdx & " and AD_IP = '" & svr_remoteAddr & "' "
	'sql = sql & " and TMP_NUM1 = dbo.ecl_ENCRPART('" & num & "',4) "
	sql = sql & " and TMP_NUM1 = '" & num & "' "
	call execSql(sql)
	
	response.write	"<script>"
	response.write	"top.fnLoadAddr(top.nTab);top.fnLoadTrg();top.fnLoadingE();"
	response.write	"</script>"
...
elseif proc = "sel" then
	...
	sql = " select '', TMP_NO, TMP_IDX, TMP_NM, dbo.ecl_DECRPART(TMP_NUM1,4), dbo.ecl_DECRPART(TMP_NUM2,4), dbo.ecl_DECRPART(TMP_NUM3,4) from TMP_CALLTRG with(nolock) "
