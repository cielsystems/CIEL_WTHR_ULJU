...
dim strMesg
dim nData, nLine
dim arrViewData(10, 4)

for dataLoop2 = 0 to dataRc2
	
	nLine	= dataLoop2 + 1
	
	for dataLoop1 = 0 to 10
	...
	if len(allData) > 0 then
		...
		for dataLoop1 = 0 to 10
			...
			if dataLoop1 = 0 then
				if len(nData) = 0 then
					chekEror	= true
					strMesg	= nLine & "번째 데이터에 부서(그룹)1이 없습니다."
					exit for
				end if
			end if
			...
			if dataLoop1 = 5 then
				if len(nData) = 0 then
					chekEror	= true
					strMesg	= nLine & "번째 데이터에 이름이 없습니다."
					exit for
				end if
			end if
			...
			if dataLoop1 = 6 then
				...
				if fnChkMobileNum(nData) = false then
					chekEror	= true
					strMesg	= nLine & "번째 데이터에 휴대폰번호가 형식에 맞지 않습니다."
					exit for
				end if
			end if
...
call subSetLog(ss_userIdx, 8004, "주소록파일업로드", "file : " & strRealFile, "")
%>